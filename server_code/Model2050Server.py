...
TEST_MODEL_DIR = BASE_DIR / "tests" / "test_model"


def _load_model_module():
    """Load the generated model interface, or fall back to the bundled demo model."""
    try:
        from . import interface2050 as generated_interface2050

        return generated_interface2050, False
    except ImportError:
        import sys

        sys.path.insert(0, str(TEST_MODEL_DIR))
        import interface2050 as demo_interface2050

        return demo_interface2050, True


INTERFACE2050, USING_DEMO_MODEL = _load_model_module()
...
table_path = (
    TEST_MODEL_DIR / "web_outputs.json"
    if USING_DEMO_MODEL
    else BASE_DIR / "web_outputs.json"
)
...
def initial_values():
    ...
    return dict(
        ...
        lever_categories=CONFIG["lever_categories"],
    )
