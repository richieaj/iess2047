    ...
    def __init__(self, **properties):
        ...
        self.lever_panel.items = [
            dict(
                ...
                tooltips=tooltips,
                value=value,
                bold = (True if name in ("Cost", "GDP") else False),
	    ...
	    # if len(self.lever_panel.items) == 1:

        if self.item["name"] == "GDP" or self.item["name"] == "Cost" :
            self.lever_panel.visible = True
            self.lever_spacer.visible = True
            self.arrow_button.visible = False
            self.group_lever.visible = False
